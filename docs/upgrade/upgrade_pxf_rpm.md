...
The current PXF code requires **Java 11** (or newer). A package upgrade may preserve an old
Java 8 `JAVA_HOME` in `$PXF_BASE/conf/pxf-env.sh`. Install Java 11 on every host,
update that path, verify `"$JAVA_HOME/bin/java" -version`, and synchronize the
configuration before starting the new service. See
